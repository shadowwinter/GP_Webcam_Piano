    ...
    for (var i=0;i<this.notePos.length;i++){
      for (var j=0;j<this.notePos[i].length;j++){
        ...
        if (this.noteState[i][j]>0) {
          ...
          if(this.isPlaying[i][j]>0){
            if(selectMode.value()=="Piano"){
              var num = int(map(sum,0,26,0,36));
              piano[num].play();
            }
            if(selectMode.value()=="Guitar"){
              var num = int(map(sum,0,26,0,9));
              guitar[num].play();
            }
            this.isPlaying[i][j]=0;
          }
  ...
  showNotes(){
      for (var i=0;i<this.notePos.length;i++){
        for (var j=0;j<this.notePos[i].length;j++){
          ...
          if(selectMode.value()=="Piano"){
            var num = int(map(sum,0,26,0,36));
            text(pianoNotes[num],x-12,y+5);
          }
          if(selectMode.value()=="Guitar"){
            var num = int(map(sum,0,26,0,9));
            text(guitarNotes[num],x-12,y+5);
          }
